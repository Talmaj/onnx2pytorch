...
from onnx2pytorch.operations.reducelogsum import ReduceLogSum


def check_reduce_log_sum(x, axes, keepdims, opset_version=13):
    inputs = ["x"]
    initializers = []
    ...
    node = helper.make_node("ReduceLogSum", inputs=inputs, outputs=["y"], **attrs)
...
def test_reduce_log_sum(opset_version, keepdims, axes):
    np.random.seed(0)
    x = np.random.uniform(0.1, 10, (3, 4, 5)).astype(np.float32)
    check_reduce_log_sum(x, axes, keepdims, opset_version)
