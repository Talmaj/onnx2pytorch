...
def test_lrn_against_onnxruntime(opset_version, size):
...
def test_lrn_defaults_against_onnxruntime():
    np.random.seed(0)
    x = np.random.randn(2, 6, 3, 3).astype(np.float32)
...
def test_lrn_even_size(size):
    ...
    exp_y = lrn_reference(x, size, alpha=0.0002, beta=0.6, bias=2.0)
    assert np.allclose(y.numpy(), exp_y, rtol=1e-5, atol=1e-6)
