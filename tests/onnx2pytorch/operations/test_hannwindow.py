...
def check_hann_window(size, out_type=TensorProto.FLOAT, **attrs):
    node = helper.make_node("HannWindow", inputs=["size"], outputs=["y"], **attrs)
...
def test_hann_window(size):
    check_hann_window(size)


@pytest.mark.parametrize("periodic", [0, 1])
def test_hann_window_periodic(periodic):
    check_hann_window(16, periodic=periodic)


def test_hann_window_double():
    check_hann_window(
