...
def check_hamming_window(size, out_type=TensorProto.FLOAT, **attrs):
    node = helper.make_node("HammingWindow", inputs=["size"], outputs=["y"], **attrs)
...
def test_hamming_window(size):
    check_hamming_window(size)


@pytest.mark.parametrize("periodic", [0, 1])
def test_hamming_window_periodic(periodic):
    check_hamming_window(16, periodic=periodic)


def test_hamming_window_double():
    check_hamming_window(
