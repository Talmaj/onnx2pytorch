...
def convert_operations(onnx_graph, opset_version, batch_dim=0, enable_pruning=True):
    ...
    for i, node in enumerate(onnx_graph.node):
        ...
        if node.op_type == "Abs":
        ...
        elif node.op_type == "CenterCropPad":
            op = CenterCropPad(**extract_attributes(node))
            ...
            if params:
                ...
                # check if next node Add to add bias (if there is another node)
            ...
            op = MaxRoiPool(**extract_attributes(node))
            ...
            op = NegativeLogLikelihoodLoss(**extract_attributes(node))
            ...
            op = QLinearConv(**extract_attributes(node))
        ...
        elif node.op_type == "TensorScatter":
            op = TensorScatter(**extract_attributes(node))
            ...
            op = Trilu(**extract_attributes(node))
        elif node.op_type == "Unique":
            op = Unique(**extract_attributes(node))
